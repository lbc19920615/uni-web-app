<template>
	<view class="waterfalls-flow">
		...
		:id="`waterfalls_flow_column_${index+1}`" :msg="msg" :style="{'width':w,'margin-left':index==0?0:m}">
			<view :class="['column-value',{'column-value-show':item2.o}]" 
...
import { deepClone } from '@/utils/clone';
	...
	export default {
		...
		methods: {
			...
			refresh() {
				if (!this.isLoaded) {
					this.refreshDatas = deepClone(this.value);
					return false;
				};
				setTimeout(() => {
					this.refreshDatas = [];
					this.isRefresh = true;
					this.adds = [];
					this.data.list = this.value ? deepClone(this.value) : [];
