...
function parseReplacements(replacements: Replacement[]): Array<
  Omit<Replacement, 'replace' | 'filter'> & {
    filter: RegExp;
    replace: ReplaceFn[];
  }
> {
        ...
        ...resolvedReplacements.map((option) => {
          return {
            ...
            setup(build: PluginBuild) {
              build.onLoad(
                { filter: option.filter, namespace: 'file' },
                async ({ path }) => {
                  const source = await fs.readFile(path, 'utf8');

                  return {
                    loader: 'default',
                    contents: option.replace.reduce(
                      (text, replace) => replace(text, path),
                      source
                    )
                  };
                }
